...
2. **Map columns**:
   - Required: `title`, `type`, and _either_ `date` (milestone) or `start_date`/`end_date` (task).
   - Optional: `owner`, `lane`, `project`, `tags`.
   - **ID strategy**:
     - _Generate new_ (UUID) — default for first import.
     - _Use column_ (e.g., “External ID”).
     - _Match keys_ (e.g., `project + title`); if a later import matches, it updates same logical row.
...
- a row _matches_ if its **computed match key** equals an existing item in `branchId`.
...
- **branch_id** is part of the primary key, so re-importing into a _simulation branch_ produces a clean, isolated history.
